...
`"GroupDescription": "Security Group for the UI Application Load Balancer"`는 UI 애플리케이션용 로드 밸런서에 연결되는 보안 그룹이라는 뜻입니다. 사용자가 웹 애플리케이션의 UI(프론트엔드)로 요청을 보내면, 사용자가 웹 애플리케이션의 UI(프론트엔드)로 요청을 보내면, 이 로드 밸런서가 해당 요청을 받아 타깃 그룹에 등록된 인스턴스나 태스크로 분산합니다.
...
보안 요구 수준이 높은 환경에서는 아웃바운드 트래픽도 필요한 목적지와 포트만 허용하도록 제한하기도 합니다.
...
`"SecurityGroupIngress"`는 이 보안 그룹이 붙은 리소스로 들어오는 트래픽에 대한 허용 규칙입니다. `"CidrIp": "0.0.0.0/0"`은 모든 IP 주소 대역을 의미하며, `"FromPort"` 값은 허용할 포트 범위의 시작 번호를, `"ToPort"` 값은 허용할 포트 범위의 끝 번호를 의미합니다. 이 설정은 모든 IP 주소 대역에서 들어오는 TCP 트래픽 중 `80`, `8090`, `8080`, `443` 포트에 대한 접근을 허용합니다.
...
메타데이터를 의미합니다. 이 메타데이터는 `cdk-nag` 경고를 처리하기 위한 설정입니다. `cdk-nag`은 AWS CDK 코드에 대해 보안 및 베스트 프랙티스 검사를 수행하는 도구입니다. `"rules_to_suppress"`는 특정 검사 규칙에서 발생하는 경고를 무시하도록 설정하는 항목입니다. 여기서는 ALB가 외부에서 접근 가능한 진입점이어야 하므로, 공개 인바운드 트래픽 허용에 대한 `cdk-nag` 경고를 무시하도록 설정했습니다. `"id": "Workshop-EC2RestrictedInbound"`는 이 워크숍 또는 관련 검사 체계에서 사용하는 규칙 식별자입니다. 이름상 공개 인바운드 허용과 관련된 경고를 가리키는 것으로 보이며, 여기서는 ALB를 외부에 공개해야 하므로 해당 경고를 예외 처리한 것으로 이해할 수 있습니다.
...
모든 IP 주소 대역에서 들어오는 TCP `8080` 포트 트래픽을 허용하도록 설정되어 있습니다.
