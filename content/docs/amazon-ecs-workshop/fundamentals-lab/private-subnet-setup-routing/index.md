...
이 프라이빗 서브넷은 첫 번째 퍼블릭 서브넷이 속한 것과 동일한 AZ에 배치됩니다.

---

`"CidrBlock": "10.0.3.0/24"`는 `10.0.3.0`부터 `10.0.3.255`까지의 IP 주소 범위를 의미합니다. 앞의 퍼블릭 서브넷들이 `10.0.1.0/24`, `10.0.2.0/24`를 사용하므로, 이 프라이빗 서브넷은 그 다음 범위인 `10.0.3.0/24`를 사용합니다.
...
라우트 테이블 어소시에이션(Route Table Association)을 생성합니다.
...
프라이빗 서브넷의 리소스(인스턴스)는 인터넷에서 직접 접근되어서는 안 되기 때문에 Public IP를 가지지 않습니다. 따라서 인터넷으로 나가는 요청은 NAT Gateway를 통해 전달됩니다. 이때 NAT Gateway는 요청 패킷의 출발지 사설 IP와 포트 번호를 자신의 Public IP와 포트 번호로 변환한 뒤 인터넷으로 전달합니다. 이후 NAT Gateway가 응답 패킷을 받으면, 이 매핑 정보를 바탕으로 해당 패킷이 원래 리소스로 돌아갈 수 있도록 전달합니다.
...
나머지 코드들도 퍼블릭 서브넷과 마찬가지로 각각 서로 다른 AZ에 속한 프라이빗 서브넷입니다. 다만 내부 설정이 동일하므로, 추가적인 코드 분석은 생략하겠습니다.
