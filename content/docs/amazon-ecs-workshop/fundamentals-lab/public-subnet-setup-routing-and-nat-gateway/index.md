...
고가용성(High Availability)을 위해 여러 서브넷을 서로 다른 AZ에 배치합니다. `"Fn::Sub"`는 문자열 안의 변수 값을 실제 값으로 바꿔 넣는 CloudFormation 함수입니다. 위 코드는 `"AWS::Region"` 값을 가져온 뒤 뒤에 `"a"`를 붙여, 예를 들어 `"ap-northeast-2a"`와 같은 가용 영역을 지정합니다.
...
참고로 인스턴스는 리소스의 한 종류입니다. 서브넷, IGW, NAT Gateway는 AWS 리소스에 해당합니다. EC2 역시 AWS 리소스이지만, 동시에 인스턴스이기도 합니다.
...
이 퍼블릭 서브넷은 특정 VPC에 소속되어 있기 때문에 `"VpcId"`를 참조(`"Ref"`)합니다.
...
라우트 테이블을 생성합니다. 라우트 테이블은 서브넷 내부 리소스가 보내는 트래픽의 경로를 안내합니다.
...
`"DependsOn"`은 이 리소스(라우트)가 생성되기 전에 해당 리소스(VPC Gateway Attachment)가 먼저 생성되어야 함을 의미합니다. VPC Gateway Attachment가 먼저 생성되었다는 것은 IGW가 VPC에 연결되었다는 뜻이며, 그래야 라우트에 정의된 규칙(모든 IPv4 대상에 대한 트래픽을 IGW로 전달하는 규칙)을 정상적으로 생성하고 적용할 수 있습니다.
...
EIP는 리소스에 떼었다 붙였다 할 수 있는 고정 Public IPv4 주소입니다. Public IP는 ISP가 할당 가능한 IP를 할당하는 것이기 때문에, Private IP와 달리 일반적인 Public IP는 리소스가 다시 생성되면 다른 값으로 할당될 수 있습니다. 반면 EIP는 고정된 주소이므로, 같은 EIP를 다시 연결하면 그 값이 바뀌지 않습니다. 예를 들어 NAT Gateway를 다시 생성하더라도, 같은 EIP를 다시 연결하면 외부에 보이는 IP 주소를 그대로 유지할 수 있습니다. 리소스가 삭제되더라도 EIP는 리소스에서 분리된 상태로 그대로 남아 있습니다. 이후 리소스를 다시 생성한 뒤, 같은 EIP를 다시 연결하면 됩니다.

이 EIP는 해당 서브넷에 직접 할당된 것처럼 보이지만, 실제로는 나중에 이 서브넷에 생성될 NAT Gateway가 사용할 수 있도록 미리 생성한 것으로 보입니다.

---

`"Domain": "vpc"`는 이 EIP를 VPC에서 사용할 주소로 할당한다는 뜻입니다. 이 설정으로 생성된 EIP는 VPC용 EIP가 되며, NAT Gateway 같은 VPC 리소스에 연결할 수 있습니다. 또한 이 경우 `"AllocationId"`가 생성되며, NAT Gateway는 이 값을 사용해 EIP를 연결합니다.
...
라우트와 라우트 테이블 어소시에이션(Route Table Association)이 먼저 생성되어, 이 서브넷이 인터넷 게이트웨이로 나갈 수 있는 퍼블릭 서브넷이 된 뒤에 NAT Gateway가 생성되도록 합니다.

---

나머지 코드들은 각각 다른 AZ에 속해 있지만, 서브넷 내부 설정이 동일한 퍼블릭 서브넷들이기 때문에 이에 대한 코드 분석은 생략하겠습니다.
