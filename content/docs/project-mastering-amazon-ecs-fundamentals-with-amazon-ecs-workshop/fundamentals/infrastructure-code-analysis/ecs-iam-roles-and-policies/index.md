...
`"ManagedPolicyArns"`에서 Managed Policy는 AWS가 이미 만들어둔 IAM Policy를 의미합니다. `"Fn::Join"` CloudFormation 함수로 이 Managed IAM Policy인 `AmazonECSTaskExecutionRolePolicy`의 ARN을 만듭니다.

참고로 `"AWS::Partition"`, 즉 AWS 파티션은 리전보다 큰 AWS 환경의 단위를 말합니다. 이 값으로는 `"aws"`, `"aws-cn"`, `"aws-us-gov"` 등이 있습니다. 일반적으로 이 값은 `"aws"`에 해당합니다.

---

`"PermissionsBoundary"`는 이 IAM Role이 가지는 최대 권한을 설정하는 필드입니다. IAM Role은 IAM Policy에 명시된 권한을 가지는데, 이 Permissions Boundary는 IAM Policy에 명시된 권한이 있더라도 이 권한의 일부를 행사하지 못하게 할 수 있습니다. 여기서는 `"PermissionsBoundary2D0B48CC"`를 참조하여 Permissions Boundary로 연결합니다.

---

`"RoleName"`은 이 IAM Role의 이름을 의미합니다. 이름은 `"retailStoreEcsTaskExecutionRole"`입니다.
...
IAM Policy를 생성하는 리소스입니다. 앞서 생성한 IAM Role에는 이미 Managed Policy가 연결되었습니다. 이 IAM Policy 또한 연결되어 총 두 개의 IAM Policy가 앞서 생성한 IAM Role에 연결됩니다.
